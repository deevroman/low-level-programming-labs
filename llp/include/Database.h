...
DbPtr Database::AllocPage(page_header header, DbSize size) {
  if (size == -1) {
...
void Database::RemoveString(DbPtr ptr) const {
  debug("TODO");
  this->file_;
}
...
Database::SchemasPageIterator::SchemasPageIterator(Database *db) {
  db_ = db;
  current_page_address_ = db->master_header_.schemas_first_page;
  current_page_offset_ = 0;
}
...
void Database::SchemasPageIterator::RemoveSchemaAndShift() {
  ...
  while (current_page_2.nxt_page != 0 || current_page_offset_2 != current_page_2.ind_last_elem) {
    ...
    db_->file_->read(buffer.get(), shift_step, current_page_address_2 + sizeof(page_header) + current_page_offset_2);
    db_->file_->write(buffer.get(), shift_step, current_page_address_1 + sizeof(page_header) + current_page_offset_1);
